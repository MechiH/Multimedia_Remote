...
int Socket::setAddress(SOCKADDR_IN &addr, const string &host, int port)
{
  ...
  if (host.empty() || !(hent = ::gethostbyname(host.c_str())))
...
int Socket::bind(const string &host, int port)
{
  ...
  // for INET4 sockets
  SOCKADDR_IN addr;
  if (setAddress(addr, host, port) < 0)
    return UnknownHost;
  // assigns the address specified by addr to sockfd (returns -1 on error, 0 otherwise)
  return ::bind(sockfd_, (const SOCKADDR *)&addr, sizeof(addr));
}
...
int ServerSocket::bind(int port, int backlog)
{
  ...
  if (::getsockname(sockfd_, (SOCKADDR *)&addr, &taille) < 0)
    return -1;
#endif
...
Socket *ServerSocket::accept()
{
  ...
  if (sock_com == INVALID_SOCKET)
    return nullptr;
#else
  // cf. man -s 3n accept, EINTR et EWOULBLOCK ne sont pas geres!
  if ((sock_com = ::accept(sockfd_, NULL, NULL)) < 0)
    return nullptr;
#endif
...
struct InputBuffer
{
  InputBuffer(size_t size) : buffer(new char[size]),
                             begin(buffer),
...
SocketBuffer::SocketBuffer(Socket *sock, size_t inSize, size_t outSize) : insize_(inSize),
                                                                          outsize_(outSize),
                                                                          insep_(-1), // means '\r' or '\n' or "\r\n"
                                                                          outsep_('\n'),
                                                                          sock_(sock),
...
SOCKSIZE SocketBuffer::readLine(string &str)
{
  ...
  if (!sock_)
    return Socket::InvalidSocket;
  if (!in_)
    in_ = new InputBuffer(insize_);

  while (true)
  {
    if (retrieveLine(str, in_->remaining))
      return str.length() + 1;
    ...
    if (received <= 0)
      return received; // -1 (error) or 0 (shutdown)
    if (retrieveLine(str, received))
      return str.length() + 1;
  }
}
...
SOCKSIZE SocketBuffer::write(const char *s, size_t len)
{
  ...
  while (begin < end)
  {
    ...
    if (sent <= 0)
      return sent; // -1 (error) or 0 (shutdown)
    begin += sent;
    totalSent += sent;
  }
  return totalSent;
